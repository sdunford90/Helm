...
const TenantDetail: React.FC = () => {
  ...
  const [achRate, setAchRate] = useState<string>(String(tenant.achRate));
  const [cardRate, setCardRate] = useState<string>(String(tenant.cardRate));
  ...
  const handleSaveFees = async () => {
    try {
      await fetch(`/api/admin/tenants/${id}`, {
        ...
        body: JSON.stringify({
          achRate: achRate ? parseFloat(achRate) : null,
          cardRate: cardRate ? parseFloat(cardRate) : null,
        }),
      });
      setFeeSaved(true);
      setTimeout(() => setFeeSaved(false), 3000);
    } catch (err) {
      console.error('Failed to save fee overrides', err);
    }
  };
    ...
    <div>
      ...
      {tab === 'overview' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20 }}>
          ...
          <div style={card}>
            <div style={{ fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 16 }}>Platform Fee Rates</div>
            {[
              ['ACH Fee Rate', `${tenant.achRate}%`],
              ['Card Fee Rate', `${tenant.cardRate}%`],
            ].map(([l, v]) => (
              <div key={l as string} style={fieldRow}>
                <span style={fieldLabel}>{l}</span>
                <span style={{ ...fieldValue, color: '#FF9800', fontWeight: 700 }}>{v}</span>
              </div>
            ))}
          </div>
          <div style={card}>
      ...
      {tab === 'subscription' && (
        <div>
          ...
          <div style={card}>
            <div style={{ fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 16 }}>Platform Fee Overrides</div>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', marginBottom: 16, fontStyle: 'italic' }}>These override the tier default. Leave blank to use tier defaults.</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              <div>
                <label style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: 1, display: 'block', marginBottom: 6 }}>ACH Fee Rate (%)</label>
                <input
                  type="number"
                  step="0.01"
                  min="0"
                  placeholder="e.g. 0.8"
                  value={achRate}
                  onChange={(e) => setAchRate(e.target.value)}
                  style={{ width: '100%', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, padding: '10px 12px', color: '#FFF', fontSize: 14, outline: 'none', boxSizing: 'border-box' }}
                />
              </div>
              <div>
                <label style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: 1, display: 'block', marginBottom: 6 }}>Card Fee Rate (%)</label>
                <input
                  type="number"
                  step="0.01"
                  min="0"
