...
export async function seedOperations(prisma: PrismaClient, tenantId: string, users: any[], slips: any[]) {
  ...
  const walks = await Promise.all([0, 1, 2, 3, 4, 5, 6, 7].map(async (dayBack) => {
    ...
    const walk = await prisma.dockWalk.create({
      data: { tenantId, inspectorId: inspector.id, dock: dayBack % 3 === 0 ? 'A' : dayBack % 3 === 1 ? 'B' : 'C', startedAt: d, completedAt: dayBack > 0 ? new Date(d.getTime() + 45 * 60000) : null, status: dayBack > 0 ? 'COMPLETED' : 'IN_PROGRESS' },
    });

    // Walk items for occupied slips in that dock
    const dockLetter = dayBack % 3 === 0 ? 'A' : dayBack % 3 === 1 ? 'B' : 'C';
    const dockSlips = slips.filter((s: any) => s.dock === dockLetter);
  ...
  const announcements = await Promise.all([
    prisma.announcement.create({ data: { tenantId, subject: 'Weekend Marina Events — March 28-29', body: 'Join us for our spring boat show and BBQ this weekend. Live music, food trucks, and vendor exhibits from 10 AM to 6 PM.', audienceFilter: 'ALL', channels: ['EMAIL', 'SMS'], isEmergency: false, sentAt: new Date('2026-03-24'), staffId: users[0].id } }),
    prisma.announcement.create({ data: { tenantId, subject: 'Dock C Maintenance — March 26', body: 'Scheduled power maintenance on Dock C from 8 AM to 12 PM on March 26. Shore power will be temporarily unavailable during this window.', audienceFilter: 'DOCK_C', channels: ['EMAIL', 'SMS'], isEmergency: false, sentAt: new Date('2026-03-23'), staffId: users[1].id } }),
    prisma.announcement.create({ data: { tenantId, subject: 'Small Craft Advisory — Afternoon Storms', body: 'The National Weather Service has issued a small craft advisory for the Tampa Bay area from 2 PM to 8 PM today. Please secure your vessels and avoid open water.', audienceFilter: 'ALL', channels: ['EMAIL', 'SMS', 'PUSH'], isEmergency: true, sentAt: new Date('2026-03-22'), staffId: users[0].id } }),
